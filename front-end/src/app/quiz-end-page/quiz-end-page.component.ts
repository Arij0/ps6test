...
export class QuizEndComponent implements OnInit {
  ...
  restartQuiz() {
    this.scoreService.resetScore(); // Quand on recommence, on remet à zéro !
    this.router.navigate(['/game']);// Redirige vers la page du jeu pour recommencer
  }

  goToNextQuiz() {
    this.router.navigate(['/next-quiz']); // Dirige vers un autre quiz si nécessaire
  }
